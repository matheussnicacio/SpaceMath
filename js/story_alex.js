...
(function () {
  ...
  var ALEX = {
    rooms: [
      ...
      },
      {
        ...
        num: 7,
        icon: "⚖️",
        name: "NÚCLEOS DE POLARIDADE",
        desc: "Equilibre o núcleo com cristais de −5 a +5 na reta numérica.",
      ...
      },
      {
        ...
        desc: "Desenhe atalhos espaciais e lance a nave no caminho certo!",
  ...
  function alexSave() {
    try {
      localStorage.setItem(
        "alexLunarStoryProgress",
        JSON.stringify({
          progress: ALEX.progress,
          stats: ALEX.stats,
        }),
      );
    } catch (_) {}
  ...
  window.openAlexStory = function () {
    ...
    try {
      if (window._spacemathPlayMusic) window._spacemathPlayMusic("menu");
    } catch (_) {}
  };
  ...
  function renderRoomMap() {
    ...
    var done = 0;
    ALEX.rooms.forEach(function (r) {
      if (ALEX.progress[r.id]) done++;
    });
    ...
    var currentIdx = ALEX.rooms.findIndex(function (r) {
      return !ALEX.progress[r.id];
    });
    ...
    ALEX.rooms.forEach(function (r, i) {
      ...
      var extraBadge =
        r.num > 5
          ? '<div class="alex-room-special-badge">ESPECIAIS</div>'
          : '<div class="alex-room-basic-badge">BÁSICAS</div>';

      card.innerHTML =
        extraBadge +
        '<div class="alex-room-icon">' +
        r.icon +
        "</div>" +
        '<div class="alex-room-info">' +
        '<div class="alex-room-num">ESTAÇÃO ' +
        r.num +
        " · " +
        r.instructor.name +
        "</div>" +
        '<div class="alex-room-name">' +
        r.name +
        "</div>" +
        '<div class="alex-room-desc">' +
        r.desc +
        "</div>" +
        "</div>" +
        statusEl;
  ...
  function dlgShowCurrent() {
    ...
    DLG.timer = setInterval(function () {
      if (DLG.charIdx >= DLG.target.length) {
        ...
        document.getElementById("alexDlgSkip").textContent =
          DLG.idx === DLG.lines.length - 1 ? "COMEÇAR ▶" : "PRÓXIMO ▶";
        return;
      }
      DLG.charIdx++;
  ...
  function enterRoom(roomId) {
    var room = ALEX.rooms.find(function (r) {
      return r.id === roomId;
    });
    if (!room) return;
    ...
    document.getElementById("alexStNumLabel").textContent = "ESTAÇÃO " + room.num + " DE 9";
  ...
  window.exitAlexStation = function () {
    document.getElementById("alexStationScreen").classList.remove("show");
    document.getElementById("alexTaskBar").style.display = "none";
    if (ALEX.rooms.every(function (r) {
      return ALEX.progress[r.id];
    })) {
      openCeremony();
      return;
  ...
  function reactorBuildStage() {
    ...
    showTaskBar(
      "Carregue o reator até <strong>" +
        REACTOR.target +
        " joules</strong>. Encontre <strong>" +
        task.minSols +
        "</strong> combinações diferentes.",
      REACTOR.taskIdx + 1,
      REACTOR.tasks.length,
    );
    ...
    stage.innerHTML =
      ...
      '<div class="reactor-target-value">' +
      REACTOR.target +
      " J</div>" +
      '<div class="reactor-current">SOMA ATUAL: <span id="reactorSum">0</span> / <span class="target-num">' +
      REACTOR.target +
      "</span></div>" +
      "</div>" +
      ...
      'Combinações descobertas: <span class="acc">0</span> / ' +
      task.minSols +
      "</div>" +
      '<div class="reactor-controls">' +
      '<button class="alex-action-btn" onclick="reactorReset()">LIMPAR</button>' +
      '<button class="alex-action-btn green btn-pulsar" onclick="reactorNext()" disabled id="reactorNextBtn">PRÓXIMA ▶</button>' +
      "</div>" +
      "</div>";
  ...
  function reactorRenderCrystals() {
    ...
    REACTOR.crystals.forEach(function (v) {
      ...
      c.innerHTML =
        '<div class="reactor-crystal-icon">' +
        (icons[v] || "💎") +
        "</div>" +
        '<div class="reactor-crystal-value">+' +
        v +
        "</div>" +
  ...
  function hangarBuildStage() {
    ...
    stage.innerHTML =
      ...
      '<div class="hangar-counter-value" id="hgTotal">' +
      t.total +
      "</div>" +
      "</div>" +
      ...
      '<div class="hangar-counter-value target" id="hgTarget">' +
      t.target +
      "</div>" +
      "</div>" +
      '<div class="hangar-counter-arrow">=</div>' +
  ...
  function hangarSetupDrag() {
    ...
    window.addEventListener("mousemove", function (e) {
      ...
      if (el && el.classList.contains("hangar-crate")) {
        ...
        var c = HANGAR.crates.find(function (x) {
          return x.id === id;
        });
        if (c && !c.gone) {
          ...
          // Update counters
          var sel = HANGAR.crates.filter(function (x) {
            return x.sel && !x.gone;
          }).length;
  ...
  function hangarRenderCrates() {
    ...
    HANGAR.crates.forEach(function (c) {
      ...
      el.innerHTML =
        '<svg viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" class="crate-svg">' +
    ...
    var sent =
      HANGAR.total -
      HANGAR.crates.filter(function (c) {
        return !c.gone;
      }).length;
    var still = HANGAR.crates.filter(function (c) {
      return !c.gone;
    }).length;
    ...
    var sel = HANGAR.crates.filter(function (c) {
      return c.sel && !c.gone;
    }).length;
    HANGAR.selected = sel;
    var btn = document.getElementById("hgDispatchBtn");
  ...
  window.hangarDispatch = function () {
    var t = HANGAR.tasks[HANGAR.taskIdx];
    var sentNow = HANGAR.crates.filter(function (c) {
      return c.sel && !c.gone;
    });
    var sentTotal =
      HANGAR.total -
    ...
    setTimeout(function () {
      ...
      var remaining = HANGAR.crates.filter(function (c) {
        return !c.gone;
      }).length;

      if (totalSent === t.target) {
        toast(
          "✓ Despachadas exatamente " + t.target + " caixas! Sobram <strong>" + remaining + "</strong>.",
          "success",
        );
        document.getElementById("hgStrategy").innerHTML =
          "🎯 Resultado: <strong>" +
          t.total +
          " − " +
          t.target +
          " = " +
          remaining +
          "</strong>. <em>Próxima missão em alguns segundos…</em>";
        ALEX.stats.totalSolutions++;
  ...
  function labBuildStage() {
    ...
    showTaskBar(
      "Sintetize exatamente <strong>" +
        t.target +
        "</strong> cristais. Encontre <strong>2+</strong> arranjos diferentes.",
      LAB.taskIdx + 1,
      LAB.tasks.length,
    );
    ...
    stage.innerHTML =
      ...
      '<button class="alex-action-btn cyan btn-pulsar" onclick="labNext()" disabled id="labNextBtn">PRÓXIMA ▶</button>' +
      "</div>" +
      "</div>";
  ...
  window.labSubmit = function () {
    if (LAB.rows * LAB.cols !== LAB.target) {
      return;
    }
  ...
  function initComms() {
    ...
    dlgStart(
      [
        ...
        "Arraste uma antena do estoque até uma cratera. Ou toque na cratera para enviar.",
      ],
      function () {
        ALEX.sessionActive = true;
        commsBuildStage();
  ...
  function commsBuildStage() {
    ...
    showTaskBar(
      "Distribua <strong>" +
        t.total +
        "</strong> antenas entre <strong>" +
        t.colonies +
        "</strong> colônias. <em>Arraste</em> do estoque até cada cratera.",
      COMMS.taskIdx + 1,
      COMMS.tasks.length,
    );
    ...
    var infoBlocks =
      ...
      '<div class="comms-info-value" id="commsLeft">' +
      t.total +
      "</div>" +
      "</div>" +
      '<div class="comms-info-block">' +
      '<div class="comms-info-label">DIVIDIR POR</div>' +
      '<div class="comms-info-value left">' +
      t.colonies +
      "</div>" +
      "</div>" +
      '<div class="comms-info-block">' +
      '<div class="comms-info-label">ALVO P/ COL.</div>' +
      '<div class="comms-info-value each">' +
      quotient +
      (rest > 0 ? "+" : "") +
      "</div>" +
      "</div>" +
      (rest > 0
        ? '<div class="comms-info-block">' +
          ...
          '<div class="comms-info-value rest">' +
          rest +
          "</div>" +
          "</div>"
        : "");
    ...
    for (var i = 0; i < t.colonies; i++) {
      ...
      coloniesHtml +=
        '<div class="comms-colony" id="commsCol' +
        i +
        '" data-colony-idx="' +
        i +
        '" onclick="commsPlace(' +
        i +
        ')">' +
        '<div class="comms-colony-icon">' +
        c.icon +
        "</div>" +
        '<div class="comms-colony-name">' +
        c.name +
        "</div>" +
        '<div class="comms-colony-count" id="commsColCount' +
        i +
        '">0</div>' +
        '<div class="comms-colony-antennas" id="commsColAnt' +
        i +
        '"></div>' +
        "</div>";
    }
    ...
    stage.innerHTML =
      ...
      "</div>" +
      '<div class="comms-colonies">' +
      coloniesHtml +
      "</div>" +
      '<div class="comms-controls">' +
      '<button class="alex-action-btn" onclick="commsReset()">REINICIAR</button>' +
      '<button class="alex-action-btn green btn-pulsar" onclick="commsNext()" disabled id="commsNextBtn">PRÓXIMA ▶</button>' +
      "</div>" +
      "</div>";
  ...
  function commsColonyAt(x, y) {
    var el = document.elementFromPoint(x, y);
    while (el) {
      if (el.classList && el.classList.contains("comms-colony")) return el;
      el = el.parentElement;
    }
    return null;
  }

  function commsClearDropHighlight() {
    document.querySelectorAll(".comms-colony.drop-target").forEach(function (c) {
      c.classList.remove("drop-target");
    });
  }

  function commsBindDragDrop() {
    var stock = document.getElementById("commsStockpile");
    if (!stock) return;

    stock.querySelectorAll(".comms-antenna").forEach(function (ant) {
      ant.style.cursor = "grab";
      ant.style.touchAction = "none";

      ant.addEventListener("pointerdown", function (e) {
        if (e.button !== 0) return;
        e.preventDefault();
        var drag = { ghost: null, source: ant };

        function onMove(ev) {
          if (!drag.ghost) {
            drag.ghost = ant.cloneNode(true);
            drag.ghost.classList.add("comms-antenna-ghost");
            document.body.appendChild(drag.ghost);
            ant.classList.add("dragging");
          }
          drag.ghost.style.left = ev.clientX - 15 + "px";
          drag.ghost.style.top = ev.clientY - 18 + "px";
          commsClearDropHighlight();
          var col = commsColonyAt(ev.clientX, ev.clientY);
        ...
        function onUp(ev) {
          ...
          ant.classList.remove("dragging");
          commsClearDropHighlight();
          if (drag.ghost) {
            drag.ghost.remove();
            var col = commsColonyAt(ev.clientX, ev.clientY);
            if (col) {
              var idx = parseInt(col.dataset.colonyIdx, 10);
              if (!isNaN(idx)) commsPlace(idx, ant);
            }
          }
        }
  ...
  function commsRenderStock() {
    ...
    if (!box) return;
    var placedSum = COMMS.placed.reduce(function (a, b) {
      return a + b;
    }, 0);
    ...
    if (left === 0) {
      box.innerHTML =
        '<div style="color:var(--text-dim);font-family:Exo 2;font-size:12px;font-style:italic;">Estoque vazio.</div>';
    }
    document.getElementById("commsLeft").textContent = left;
    commsBindDragDrop();
  }

  window.commsPlace = function (idx, antennaEl) {
    var placedSum = COMMS.placed.reduce(function (a, b) {
      return a + b;
    }, 0);
    ...
    if (ants.length) {
      var target = antennaEl && Array.prototype.indexOf.call(ants, antennaEl) >= 0 ? antennaEl : ants[ants.length - 1];
      ...
      if (rest === 0 && minCount === maxCount) {
        ...
        toast(
          "🎯 Divisão com resto! " +
            ...
            "</strong> + resto <strong>" +
            rest +
            "</strong>. Algumas colônias ficaram com 1 a mais.",
          "success",
        );
        ALEX.stats.totalSolutions++;
        document.getElementById("commsNextBtn").disabled = false;
  ...
  function trainingBuildStage() {
    ...
    showTaskBar(
      "Construa expressões que resultem em <strong>" +
        t.target +
    ...
    stage.innerHTML =
      ...
      '<div class="training-target-value">' +
      TRAIN.target +
      "</div>" +
      "</div>" +
      ...
      '<button class="alex-action-btn green btn-pulsar" onclick="trainingNext()" disabled id="trainNextBtn">PRÓXIMA ▶</button>' +
      "</div>" +
      "</div>";
  }
  ...
  function trainingRender() {
    ...
    var pretty = TRAIN.expr.replace(/\*/g, "×").replace(/-/g, "−");
  ...
  window.trainingTest = function () {
    if (TRAIN.expr === "") {
      toast("Digite uma expressão antes!", "warn");
      return;
    }
    ...
    if (!/[\+\-\*\/]/.test(TRAIN.expr)) {
      toast("⚠ Use pelo menos uma operação na expressão!", "warn");
      return;
    }
    var v;
    try {
      ...
      toast("⚠ Expressão mal-formada. Verifique parênteses.", "error");
      return;
    }
    if (typeof v !== "number" || !isFinite(v)) {
      toast("⚠ Resultado inválido (talvez divisão por zero?).", "error");
      return;
    }
    if (Math.abs(v - TRAIN.target) > 1e-9) {
      toast(
        "= " + (Number.isInteger(v) ? v : v.toFixed(2)) + ". Não chegou em " + TRAIN.target + ". Tente outra!",
        "info",
      );
      return;
    }
    ...
    TRAIN.solutions.forEach(function (s) {
      ...
      c.textContent = s.replace(/\*/g, "×").replace(/-/g, "−") + " = " + TRAIN.target;
      chips.appendChild(c);
    });

    // Patente
    var rank = "CADETE";
    var rankColor = "var(--text-dim)";
    if (n >= 5) {
      rank = "CAPITÃO";
      rankColor = "var(--yellow)";
    } else if (n >= 3) {
      rank = "SARGENTO";
      rankColor = "var(--green)";
    } else if (n >= 2) {
    ...
    rkEl.innerHTML =
      'PATENTE ATUAL: <span class="num" style="color:' +
      rankColor +
      '">' +
      rank +
      "</span> — " +
    ...
    if (n === 1) {
      toast("🎯 PRIMEIRA SOLUÇÃO! Patente: SOLDADO.", "success");
    } else if (n === t.minSols) {
      ...
      toast("+1! Total: " + n + ". Patente: " + rank + ".", "success");
    } else {
      toast("+1! Total: " + n + ". Faltam " + (t.minSols - n) + " p/ avançar.", "success");
    }
    ...
    if (TRAIN.taskIdx < TRAIN.tasks.length) {
      ...
      dlgStart(
        [
          ...
          "<strong>Está pronto.</strong> Volte ao mapa e continue sua jornada.",
        ],
        function () {
          document.getElementById("alexTaskBar").style.display = "none";
          exitAlexStation(); // Retorna automaticamente
  ...
  function fractionsBuildStage() {
    ...
    showTaskBar(
      "Pinte <strong>" + t.num + "/" + t.den + "</strong> das " + t.parts + " partes.",
      FRACTIONS.taskIdx + 1,
      FRACTIONS.tasks.length,
    );
    var stage = document.getElementById("alexStationStage");
    stage.innerHTML =
      '<div class="fractions-stage">' +
      '<div class="fractions-display"><div class="fractions-target"><span class="num">' +
      t.num +
      '</span><span class="bar"></span><span class="den">' +
      t.den +
      "</span></div>" +
      '<div class="fractions-desc">de ' +
      t.parts +
      ' partes = <span id="fractCount">0</span> partes</div></div>' +
  ...
  function fractionsRender() {
    ...
    for (var i = 0; i < n; i++) {
      ...
      var d = ["M", centerX, centerY, "L", x1, y1, "A", radius, radius, 0, largeArc, 1, x2, y2, "Z"].join(" ");
  ...
  window.fractionsCheck = function () {
    if (FRACTIONS.selectedParts === (FRACTIONS.target.num / FRACTIONS.target.den) * FRACTIONS.totalParts) {
      toast("🎉 EXATO!", "success");
      ALEX.stats.totalSolutions++;
      setTimeout(fractionsNext, 1200);
    } else {
      toast("Fração incorreta. Tente pensar na proporção.", "warn");
    }
  };
  ...
  // ═════════════════════════════════════════════════════════════════════════
  // ESTAÇÃO 7: NÚCLEOS DE POLARIDADE (Inteiros — linha numérica como Reator)
  // ═════════════════════════════════════════════════════════════════════════
  var INTEGERS = {
    current: 0,
    history: [],
    foundSolutions: [],
    crystals: [-5, -3, -2, -1, 0, 1, 2, 3, 5],
    target: 0,
    taskIdx: 0,
    tasks: [],
    minSolutions: 2,
  ...
  function integersGetLineRange() {
    var t = INTEGERS.target;
    return {
      min: Math.min(-15, t - 5),
      max: Math.max(10, t + 5),
    };
  ...
  function integersValToPct(v) {
    var range = integersGetLineRange();
    ...
    INTEGERS.tasks = [
      {
        target: -7,
        minSols: 2,
        hint: "Use cristais negativos. Tente combinações diferentes!",
      },
      {
        target: 4,
        minSols: 2,
        hint: "Misture positivos e negativos para chegar em +4.",
      },
      {
        target: -11,
        minSols: 3,
        hint: "O alvo mais difícil. Planeje antes de tocar!",
      },
    ];
    INTEGERS.taskIdx = 0;
    INTEGERS.foundSolutions = [];

    dlgStart(
      [
        "Eu sou o <strong>Engenheiro Ion</strong>. Bem-vindo aos <strong>Núcleos de Polaridade</strong>.",
        "Igual ao Reator Energético — mas agora a escala vai do <strong>negativo ao positivo</strong>.",
        "Cada cristal carrega um valor: <strong>−5, −3, −2, −1, 0, +1, +2, +3 ou +5 joules</strong>.",
        'O alvo pode ser negativo ou positivo. <span class="acc-orange">Não existe uma única resposta</span> — descubra várias combinações!',
        "Use cristais vermelhos (negativos) e azuis (positivos). <strong>Descubra seus próprios caminhos.</strong>",
      ],
      function () {
        ALEX.sessionActive = true;
        integersBuildStage();
  ...
  function integersBuildStage() {
    var task = INTEGERS.tasks[INTEGERS.taskIdx];
    INTEGERS.target = task.target;
    INTEGERS.current = 0;
    INTEGERS.history = [];
    INTEGERS.foundSolutions = [];
    INTEGERS.minSolutions = task.minSols;

    showTaskBar(
      "Carregue o núcleo até <strong>" +
        formatSigned(INTEGERS.target) +
        " joules</strong>. Encontre <strong>" +
        task.minSols +
        "</strong> combinações diferentes.",
      INTEGERS.taskIdx + 1,
      INTEGERS.tasks.length,
    );

    var stage = document.getElementById("alexStationStage");
    stage.innerHTML =
      '<div class="reactor-stage integers-reactor">' +
      '<div class="reactor-target-display">' +
      '<div class="reactor-target-label">▼ ALVO ▼</div>' +
      '<div class="reactor-target-value">' +
      formatSigned(INTEGERS.target) +
      " J</div>" +
      '<div class="reactor-current">SOMA ATUAL: <span id="integersSum">0</span> / <span class="target-num">' +
      formatSigned(INTEGERS.target) +
      "</span></div>" +
      "</div>" +
      '<div class="reactor-numberline-wrap">' +
      '<div class="reactor-numberline signed-line" id="integersLine"></div>' +
      "</div>" +
      '<div class="reactor-history" id="integersHistory">' +
      '<div class="reactor-history-empty">Toque nos cristais abaixo para somá-los ↓</div>' +
      "</div>" +
      '<div class="reactor-crystals-row" id="integersCrystals"></div>' +
      '<div class="reactor-solutions-found" id="integersSolFound">' +
      'Combinações descobertas: <span class="acc">0</span> / ' +
      task.minSols +
      "</div>" +
      '<div class="reactor-controls">' +
      '<button class="alex-action-btn" onclick="integersReset()">LIMPAR</button>' +
      '<button class="alex-action-btn green btn-pulsar" onclick="integersNext()" disabled id="integersNextBtn">PRÓXIMA ▶</button>' +
      "</div>" +
      "</div>";

    integersRenderLine();
    integersRenderCrystals();
  }

  function integersRenderLine() {
    var line = document.getElementById("integersLine");
    if (!line) return;
    line.innerHTML = "";
    var range = integersGetLineRange();
    INTEGERS.lineMin = range.min;
    INTEGERS.lineMax = range.max;
    ...
    var fill = document.createElement("div");
    fill.className = "reactor-numberline-fill signed";
    fill.id = "integersFill";
    line.appendChild(fill);

    for (var v = range.min; v <= range.max; v++) {
      var tick = document.createElement("div");
      var isMajor = v % 5 === 0 || v === 0;
      tick.className =
        "reactor-numberline-tick" +
        (isMajor ? " major" : "") +
        (v === 0 ? " zero" : "");
      tick.style.left = integersValToPct(v) + "%";
      line.appendChild(tick);
      if (isMajor) {
        var lbl = document.createElement("div");
        lbl.className = "reactor-numberline-label" + (v === 0 ? " zero-label" : "");
        lbl.textContent = formatSigned(v);
        lbl.style.left = integersValToPct(v) + "%";
        line.appendChild(lbl);
      }
    }
    ...
    var t = document.createElement("div");
    t.className = "reactor-numberline-target";
    t.style.left = integersValToPct(INTEGERS.target) + "%";
    line.appendChild(t);

    var m = document.createElement("div");
    m.className = "reactor-numberline-marker";
    m.id = "integersMarker";
    m.style.left = integersValToPct(0) + "%";
    line.appendChild(m);
  }

  function integersRenderCrystals() {
    var row = document.getElementById("integersCrystals");
    if (!row) return;
    row.innerHTML = "";
    var icons = { 5: "🌟", 3: "🔵", 2: "🔷", 1: "💎", 0: "⚪" };
    INTEGERS.crystals.forEach(function (v) {
      var c = document.createElement("div");
      var cls = v < 0 ? " neg" : v > 0 ? " pos" : " zero";
      c.className = "reactor-crystal" + cls;
      c.innerHTML =
        '<div class="reactor-crystal-icon">' +
        (icons[Math.abs(v)] || "💎") +
        "</div>" +
        ...
        formatSigned(v) +
        "</div>" +
        '<div class="reactor-crystal-add">' +
        (v >= 0 ? "+" : "−") +
        "</div>";
      c.onclick = function () {
        integersAddCrystal(v);
      };
      row.appendChild(c);
    });
  }

  window.integersAddCrystal = function (value) {
    var range = integersGetLineRange();
    var next = INTEGERS.current + value;
    if (next < range.min || next > range.max) {
      toast("⚠ Cuidado — você sairia muito do alvo!", "warn");
      return;
    }
    INTEGERS.current = next;
    INTEGERS.history.push(value);
    integersUpdateDisplay();

    if (INTEGERS.current === INTEGERS.target) {
      integersRegisterSolution();
    } else if (Math.abs(INTEGERS.current - INTEGERS.target) > 5) {
      toast("Passou do alvo! Use LIMPAR para tentar de novo.", "warn");
    }
  };

  function integersUpdateDisplay() {
    document.getElementById("integersSum").textContent = formatSigned(INTEGERS.current);

    var zeroPct = integersValToPct(0);
    var curPct = integersValToPct(INTEGERS.current);
    var fill = document.getElementById("integersFill");
    var marker = document.getElementById("integersMarker");
    if (fill) {
      if (INTEGERS.current >= 0) {
        fill.style.left = zeroPct + "%";
        fill.style.width = Math.max(0, curPct - zeroPct) + "%";
      } else {
        fill.style.left = curPct + "%";
        fill.style.width = Math.max(0, zeroPct - curPct) + "%";
      }
    }
    if (marker) marker.style.left = curPct + "%";

    var hist = document.getElementById("integersHistory");
    if (INTEGERS.history.length === 0) {
      hist.innerHTML =
        '<div class="reactor-history-empty">Toque nos cristais abaixo para somá-los ↓</div>';
    } else {
      hist.innerHTML = "";
      INTEGERS.history.forEach(function (v, i) {
        if (i > 0) {
          var plus = document.createElement("span");
          plus.className = "reactor-history-plus";
          plus.textContent = "+";
        ...
        var chip = document.createElement("div");
        chip.className = "reactor-history-chip" + (v < 0 ? " neg" : v > 0 ? " pos" : "");
        chip.innerHTML =
          formatSigned(v) +
          ' <span class="reactor-history-chip-x" onclick="integersRemoveAt(' +
          i +
          ')">✕</span>';
        hist.appendChild(chip);
      });
      ...
      eq.className = "reactor-history-plus";
      eq.style.color =
        INTEGERS.current === INTEGERS.target ? "var(--green)" : "var(--yellow)";
      eq.textContent = "= " + formatSigned(INTEGERS.current);
      hist.appendChild(eq);
    }
  }

  window.integersRemoveAt = function (idx) {
    var v = INTEGERS.history[idx];
    INTEGERS.history.splice(idx, 1);
    INTEGERS.current -= v;
    integersUpdateDisplay();
  };

  function integersRegisterSolution() {
    var sig = INTEGERS.history
      .slice()
      .sort(function (a, b) {
        return a - b;
      })
      .join(",");
    var isNew = INTEGERS.foundSolutions.indexOf(sig) === -1;
    if (isNew) {
      INTEGERS.foundSolutions.push(sig);
      ALEX.stats.totalSolutions++;
      var task = INTEGERS.tasks[INTEGERS.taskIdx];
      var n = INTEGERS.foundSolutions.length;
      document.querySelector("#integersSolFound .acc").textContent = n;

      if (n >= task.minSols) {
        toast(
          "🎉 PERFEITO! Você descobriu " + n + " combinações. Próxima missão liberada!",
          "success",
        );
        document.getElementById("integersNextBtn").disabled = false;
        setTimeout(integersNext, 1200);
      } else if (n === 1) {
        toast("✓ Combinação registrada! Limpe e tente de outra forma.", "success");
      } else {
        toast("✓ Combinação " + n + "! Falta(m) " + (task.minSols - n) + ".", "success");
      ...
      setTimeout(function () {
        if (INTEGERS.current === INTEGERS.target) integersReset();
      }, 900);
    } else {
      toast("Você já descobriu essa! Tente uma combinação diferente.", "info");
      setTimeout(integersReset, 800);
    }
  }

  window.integersReset = function () {
    INTEGERS.current = 0;
    INTEGERS.history = [];
    integersUpdateDisplay();
  };

  window.integersNext = function () {
    INTEGERS.taskIdx++;
    if (INTEGERS.taskIdx < INTEGERS.tasks.length) {
      integersBuildStage();
      toast("🚀 Tarefa " + (INTEGERS.taskIdx + 1) + " iniciada!", "info");
    } else {
      completeStation("integers");
      ALEX.sessionActive = false;
      document.getElementById("alexStationStage").innerHTML = "";
      dlgStart(
        [
          "Excelente, cadete! Você dominou <strong>positivos e negativos</strong> na mesma escala.",
          "Lembre: somar um negativo é como <em>retirar energia</em>. Somar um positivo é <em>adicionar</em>.",
          '<span class="acc-green">A reta numérica não para no zero — ela continua para os dois lados.</span>',
        ],
        function () {
          document.getElementById("alexTaskBar").style.display = "none";
          exitAlexStation();
    ...
    tasks: [
      {
        ...
        b: 4,
        c: 5,
        mission: "A Base Lunar quer mandar um presente 🎁 pro Planeta Zig-Zag!",
        planet: "Planeta Zig-Zag",
      },
      {
        a: 6,
        b: 8,
        c: 10,
        mission: "Missão especial: entregar pizza espacial 🍕 na Estação Neon!",
        planet: "Estação Neon",
      },
    ],
  };
  ...
  function initGeometry() {
    ...
    dlgStart(
      [
        "Eiii, cadete! 🗺️ Eu sou o <strong>Tenente Axis</strong>, mestre dos mapas estelares!",
        "Olha só: ir reto e depois virar é <em>mais longo</em> do que voar na diagonal — o <strong>atalho mágico</strong>! ✨",
        "Dois lados formam um cantinho de 90° (um <strong>triângulo retângulo</strong>). O atalho é o lado maior: a <strong>hipotenusa</strong>.",
        'Segredo de piloto: <span class="acc-orange">a² + b² = c²</span>. Soa difícil? É só somar quadrados e tirar raiz!',
        "Calcule o atalho, digite o número e aperta <strong>HIPERSALTO</strong>! 🚀",
      ],
      function () {
        ALEX.sessionActive = true;
        geometryBuildStage();
  ...
  function geometryBuildStage() {
    if (GEOM.animFrame) {
      cancelAnimationFrame(GEOM.animFrame);
      GEOM.animFrame = null;
    }
    var t = GEOM.tasks[GEOM.taskIdx];
    showTaskBar(
      t.mission + " Calcule o <strong>atalho direto</strong> (hipotenusa).",
      GEOM.taskIdx + 1,
      GEOM.tasks.length,
    );
    var stage = document.getElementById("alexStationStage");
    stage.innerHTML =
      ...
      '<div class="geom-mission-card">' +
      '<span class="geom-mission-emoji">🛸</span>' +
      '<div class="geom-mission-text">' +
      t.mission +
      "</div>" +
      "</div>" +
      '<div class="geometry-canvas-wrap">' +
      '<canvas id="geomCanvas" width="360" height="240"></canvas>' +
      '<div class="geom-map-badge start">🏠 BASE</div>' +
      '<div class="geom-map-badge end">🪐 ' +
      t.planet +
      "</div>" +
      "</div>" +
      '<div class="geom-legend">' +
      '<span class="geom-legend-item long">🐌 Caminho comprido (A + B)</span>' +
      '<span class="geom-legend-item short">✨ Atalho turbo (C = ?)</span>' +
      "</div>" +
      '<div class="geom-sides-row">' +
      '<div class="geom-side-card horizontal">' +
      '<div class="geom-side-icon">➡️</div>' +
      '<div class="geom-side-label">Lado A</div>' +
      '<div class="geom-side-value">' +
      t.a +
      "</div>" +
      '<div class="geom-side-hint">voo reto</div>' +
      "</div>" +
      '<div class="geom-side-card vertical">' +
      '<div class="geom-side-icon">⬆️</div>' +
      '<div class="geom-side-label">Lado B</div>' +
      '<div class="geom-side-value">' +
      t.b +
      "</div>" +
      '<div class="geom-side-hint">voo de lado</div>' +
      "</div>" +
      '<div class="geom-side-card mystery">' +
      '<div class="geom-side-icon">🚀</div>' +
      '<div class="geom-side-label">Atalho C</div>' +
      '<div class="geom-side-value">?</div>' +
      '<div class="geom-side-hint">hipersalto!</div>' +
      "</div>" +
      "</div>" +
      '<div class="geometry-inputs">' +
      '<label for="geomIn">✨ Quantos passos tem o ATALHO?</label>' +
      '<div class="geom-input-row">' +
      '<input type="text" inputmode="numeric" pattern="[0-9]*" id="geomIn" class="geom-input" placeholder="?" oninput="this.value = this.value.replace(/[^0-9]/g, \'\'); geometryOnInputUpdate()">' +
      "</div>" +
      '<button class="alex-action-btn green btn-pulsar geom-launch-btn" id="geomLaunchBtn" onclick="geometryCheck()">🚀 ATIVAR HIPERSALTO!</button>' +
      "</div>";
    GEOM.rocketT = 0;
    GEOM.launching = false;
    GEOM.previewC = null;
    geometryDraw();
    geometryStartIdleAnim();
  }

  function geometryComputePreview(L, v) {
    var targetAngle = Math.atan2(L.by - L.cy, L.bx - L.cx);
    if (v <= 0) {
      return { rx: L.cx, ry: L.cy, angle: 0, ratio: 0, state: "idle" };
    }
    var ratio = v / L.t.c;
    var angle;
    if (ratio <= 1) {
      // Do voo reto (0) até o ângulo certo do atalho
      angle = targetAngle * ratio;
    } else {
      // Passou do alvo: inclinação ainda maior (visualiza "passou reto")
      angle = targetAngle * (1 + Math.min((ratio - 1) * 0.55, 0.45));
    }
    var dist = v * L.scale;
    var state = v === L.t.c ? "perfect" : v < L.t.c ? "short" : "long";
    return {
      rx: L.cx + Math.cos(angle) * dist,
      ry: L.cy + Math.sin(angle) * dist,
      angle: angle,
      ratio: ratio,
      state: state,
    };
  ...
  window.geometryOnInputUpdate = function () {
    var el = document.getElementById("geomIn");
    var status = document.getElementById("geomPreviewStatus");
    if (!el) return;
    var raw = el.value.trim();
    GEOM.previewC = raw === "" ? null : parseInt(raw, 10);
    if (isNaN(GEOM.previewC)) GEOM.previewC = null;

    if (status) {
      status.className = "geom-preview-status";
      if (GEOM.previewC === null) {
        status.textContent = "Digite um número e observe a nave! 👀";
      } else if (GEOM.previewC === GEOM.tasks[GEOM.taskIdx].c) {
        status.textContent = "✨ Perfeito! A nave aponta direto pro planeta!";
        status.classList.add("perfect");
      } else if (GEOM.previewC < GEOM.tasks[GEOM.taskIdx].c) {
        status.textContent = "🐌 Curto demais! A inclinação ainda não chega...";
        status.classList.add("short");
      } else {
        status.textContent = "💫 Passou reto! O atalho ficou grande demais...";
        status.classList.add("long");
      }
    }
  };

  function geometryStartIdleAnim() {
    if (GEOM.animFrame) cancelAnimationFrame(GEOM.animFrame);
    function loop() {
      if (!document.getElementById("geomCanvas") || GEOM.launching) return;
      geometryDraw();
      GEOM.animFrame = requestAnimationFrame(loop);
    }
    GEOM.animFrame = requestAnimationFrame(loop);
  }

  function geometryGetLayout() {
    var t = GEOM.tasks[GEOM.taskIdx];
    var ox = 55;
    var oy = 195;
    var scale = Math.min(26, 230 / t.a, 150 / t.b);
    return {
      t: t,
      ox: ox,
      oy: oy,
  ...
  function geometryDraw() {
    ...
    // Estrelas piscando
    for (var i = 0; i < 40; i++) {
      var sx = (i * 97 + GEOM.taskIdx * 13) % w;
      var sy = (i * 53 + 17) % h;
      var twinkle = 0.35 + 0.65 * Math.abs(Math.sin(Date.now() / 700 + i));
    ...
    // Nebulosa suave
    var neb = ctx.createRadialGradient(w * 0.75, h * 0.25, 0, w * 0.75, h * 0.25, 90);
    neb.addColorStop(0, "rgba(168,85,247,0.18)");
    neb.addColorStop(1, "transparent");
    ctx.fillStyle = neb;
    ctx.fillRect(0, 0, w, h);

    // Caminho longo (pernas do triângulo)
    ctx.lineCap = "round";
    ctx.lineJoin = "round";
    ...
    ctx.lineWidth = 2;
    ctx.beginPath();
    ctx.moveTo(L.ax - cs, L.ay);
    ctx.lineTo(L.ax - cs, L.ay - cs);
    ctx.lineTo(L.ax, L.ay - cs);
    ctx.stroke();
    ...
    ctx.fillText("A = " + L.t.a, (L.cx + L.ax) / 2, L.ay + 22);
    ctx.fillStyle = "#ffb070";
    ctx.fillText("B = " + L.t.b, L.ax + 22, (L.ay + L.by) / 2);

    ctx.fillStyle = "#ffd700";
    ctx.font = "bold 12px 'Press Start 2P', monospace";
    ctx.fillText("C = ?", (L.cx + L.bx) / 2 - 18, (L.cy + L.by) / 2 - 12);

    // Base e planeta
    ctx.font = "28px serif";
    ctx.textAlign = "center";
    ctx.fillText("🏠", L.cx - 8, L.cy + 10);
    ctx.fillText("🪐", L.bx + 6, L.by - 8);

    // Foguete (preview ao digitar ou animação de lançamento)
    var targetAngle = Math.atan2(L.by - L.cy, L.bx - L.cx);
    var rx, ry, angle, previewState;

    if (GEOM.launching || GEOM.rocketT > 0) {
      rx = L.cx + (L.bx - L.cx) * GEOM.rocketT;
      ry = L.cy + (L.by - L.cy) * GEOM.rocketT;
      angle = targetAngle;
    } else if (GEOM.previewC !== null && GEOM.previewC > 0) {
      var prev = geometryComputePreview(L, GEOM.previewC);
      rx = prev.rx;
      ry = prev.ry;
      angle = prev.angle;
      previewState = prev.state;

      var previewColor =
        previewState === "perfect"
          ? "rgba(57,255,20,0.95)"
          : previewState === "short"
            ? "rgba(255,140,0,0.85)"
            : "rgba(255,34,68,0.85)";
  ...
  window.geometryCheck = function () {
    var t = GEOM.tasks[GEOM.taskIdx];
    var input = document.getElementById("geomIn");
    var btn = document.getElementById("geomLaunchBtn");
    var v = parseInt(input.value, 10);
    if (isNaN(v)) {
      toast("Digite um número no atalho! 🚀", "info");
      return;
    }
    if (v === t.c) {
      if (btn) btn.disabled = true;
      toast("✨ Cálculo certo! Preparando hipersalto...", "success");
      geometryAnimateLaunch(function () {
        toast("🚀 WOOOOSH! A nave chegou voando no " + t.planet + "!", "success");
        ALEX.stats.totalSolutions++;
        setTimeout(geometryNext, 1400);
      });
    } else {
      var fails = [
        "Ops! Quase bateu num asteroide ☄️ Tenta outra distância!",
        "Hmm, esse atalho ficou curto demais! 📐 Usa a² + b² = c²",
        "A nave ficou tonta! 🌀 " + t.hint,
        "Ten. Axis diz: o atalho é sempre o lado MAIS LONGO!",
      ];
      toast(fails[Math.floor(Math.random() * fails.length)], "warn");
      input.classList.add("geom-input-shake");
      setTimeout(function () {
        input.classList.remove("geom-input-shake");
      }, 500);
    }
  };

  function geometryAnimateLaunch(done) {
    GEOM.launching = true;
    if (GEOM.animFrame) cancelAnimationFrame(GEOM.animFrame);
    var start = performance.now();
    var duration = 1200;
    function step(now) {
      GEOM.rocketT = Math.min(1, (now - start) / duration);
      geometryDraw();
      if (GEOM.rocketT < 1) {
        GEOM.animFrame = requestAnimationFrame(step);
      } else {
        GEOM.launching = false;
        if (done) done();
      }
  ...
  function geometryNext() {
    GEOM.taskIdx++;
    if (GEOM.taskIdx < GEOM.tasks.length) {
      geometryBuildStage();
      toast("🗺️ Nova missão no mapa!", "info");
    } else {
      completeStation("geometry");
      ALEX.sessionActive = false;
      document.getElementById("alexStationStage").innerHTML = "";
      dlgStart(
        [
          "UAUUU! 🎉 Você virou piloto de atalhos!",
          "Sempre que precisar da distância direta, lembre do triângulo: <strong>a² + b² = c²</strong>.",
          '<span class="acc-green">Atalho calculado = nave feliz!</span> Próxima parada: laboratório de fórmulas.',
        ],
        function () {
          document.getElementById("alexTaskBar").style.display = "none";
          exitAlexStation();
  ...
  var ALGEBRA = {
    ...
    tasks: [
      ...
      },
      {
        ...
        target: 8,
        hint: "Quase lá! Deixe o X sozinho.",
      },
    ],
  };
  ...
  function algebraBuildStage() {
    ...
    stage.innerHTML =
      ...
      '<div class="algebra-scale-plate right"><div class="label">' +
      t.target +
      "</div></div>" +
      "</div>" +
      "</div>" +
      '<div class="algebra-eq">' +
      t.eq +
      "</div>" +
  ...
  window.algebraCheck = function () {
    ...
    if (v === t.x) {
      toast("🧪 SÍNTESE BEM SUCEDIDA!", "success");
      ALEX.stats.totalSolutions++;
      setTimeout(algebraNext, 1200);
    } else {
      toast("Valor de X incorreto.", "warn");
    }
  };
  ...
  function algebraNext() {
    ...
    if (ALGEBRA.taskIdx < ALGEBRA.tasks.length) algebraBuildStage();
      ...
      completeStation("algebra");
      ALEX.sessionActive = false;
      document.getElementById("alexStationStage").innerHTML = "";
      var allDone = ALEX.rooms.every(function (r) {
        return ALEX.progress[r.id];
      });
      toast(
        allDone
          ? "🎓 Todas as estações concluídas! Cerimônia a caminho..."
          : "🧪 Síntese concluída! Estação finalizada.",
        "success",
      );
      setTimeout(exitAlexStation, allDone ? 1400 : 800);
    }
  }
  ...
  function openCeremony() {
    ...
    document.getElementById("alexCeremonyStats").innerHTML =
      ...
      '<div class="ceremony-stat-value">' +
      Object.values(ALEX.progress).filter(function (v) {
        return v;
      }).length +
      " / " +
      ALEX.rooms.length +
    ...
    try {
      if (window._spacemathPlayMusic) window._spacemathPlayMusic("victory");
    } catch (_) {}
  }
